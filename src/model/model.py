...
class Xception56(nn.Sequential):
    def __init__(self, config, classify=True):
        super(Xception56, self).__init__()
    ...
    def forward(self, inputs):
        return super(Xception56, self).__init__(inputs)
...
model= Xception56(config, classify=True)
print(model(torch.randn(1,3,224,224)).shape)

##########Xception71

class Xception71(nn.Sequential):
    def __init__(self, config, classify=True):
    ...
    def forward(self, inputs):
        return super(Xception71, self).__init__(inputs)
...
model = Xception71(config, classify=True)
print(model(torch.randn(1, 3, 224, 224)).shape)
